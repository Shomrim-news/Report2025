<script>
  ...
  const bandDefs = [
    ...
    { label: 'Investigative', test: (s) => s['Story type']?.trim() === 'Investigative' },
    {
      ...
      test: (s) => s['Story type']?.trim() === 'Short-Term Investigative',
    },
  ];
  ...
  const exceptionAtomIndices = [
    atoms.findIndex((a) => a.story['_2025'] === '105'), // Feature with Investigative Elements → drifts toward Investigative
    atoms.findIndex((a) => a.story['_2025'] === '152'), // Short-Term Inv + Follow-Up → drifts toward Short-Term Investigative
  ].filter((i) => i !== -1);
  ...
  $effect(() => {
    ...
    if (!isActive) {
      if (wasThemesActive) {
        // Fade all theme layers out, default layers back in
